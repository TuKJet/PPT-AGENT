...
- Carries the exact `source_image_path` that must be passed directly to the model and the exact pure-vector SVG `target_path`.

`img-svg/*.svg`

- One model-reconstructed, PowerPoint-compatible 1280x720 SVG for every IMG page.
- Must be pure vector: no `<image>`, raster data URI, external URL/file, `<foreignObject>`, or script.
...
- no native-SVG PPTX export until every model-generated SVG passes the one-to-one, XML, viewBox, and pure-vector checks
