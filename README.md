...
With `on`, read [`references/img-svg.md`](.codex/skills/ppt-deck-workflow/references/img-svg.md) and use [`references/img-svg-prompt.md`](.codex/skills/ppt-deck-workflow/references/img-svg-prompt.md) as the sole model prompt. Pass each source image and compiled prompt together in one vision-model turn; the image is the visual truth and the task is tracing, not redesign. The model may choose a faithful vector trace or tight source crop per element. Keep visible text as SVG text when fidelity permits. The page manifest keeps a lightweight repeated `visible_text` list; a version-4 crop manifest is optional and exists only when crops are used. `complete-img-svg` first creates `pending_visual_review` records and renders changed pages; unchanged pages reuse previews unless `--force-render` is requested. Inspect every source/render pair, repair drift, mark passed reviews with `text_checked: true`, and rerun completion before export. Do not create layered SVG variants. The final `<topic>-img-svg.pptx` embeds native SVG media rather than rasterizing the reconstructed page.
...
```bash
uv run python -u .codex/skills/ppt-deck-workflow/scripts/embed_img_crops.py --manifest output/.../img-svg/slide-01-crops.json
```
