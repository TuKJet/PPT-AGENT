...
With `on`, every version-2 `render-jobs/img-svg/slide-xx.json` contains the actual `source_image_path`, a fully compiled high-fidelity tracing prompt, prompt hash, conversion-evidence path, crop-strategy manifest, and fidelity-review paths. Pass the image and compiled prompt together in the same vision-model turn; the image is the sole visual truth and the task is tracing, not redesign. Preserve all original icons by faithful path tracing or direct Pillow crops, but use crops only for tight incompatible artwork: visible text, labels, captions, legends, cards, panels, and chart bands must remain SVG. Crop entries declare `content_type` and `contains_text: false`; deterministic validation rejects broad crops and crops that overlap vector text. Complete the evidence/manifests, then run `complete-img-svg` once to render source-comparison PNGs and pending review files. Inspect every source/render pair, repair drift, mark faithful pages passed, and rerun completion before export. Do not create `v1`, `v2`, overlay, image-elements, or clean SVG variants. The final `<topic>-img-svg.pptx` embeds native SVG media rather than rasterizing the reconstructed page.
