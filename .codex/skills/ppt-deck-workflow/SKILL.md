...
10. Preserve every visible logo, icon, badge, illustration, and decorative symbol. First complete the `visual_element_inventory`. Icons, logos, illustrations, photos, and textures default unconditionally to tight, text-free source crops embedded as Base64 PNG `<image data-crop-id="...">` nodes; do not redraw them as approximate vectors. Keep text, lines, boxes, dividers, arrows, and simple geometry vector. A simple badge or decorative symbol may use `faithful_vector_trace` only after direct comparison establishes near-pixel fidelity. Never crop a card/panel/title/chart band or visible text. Each crop must declare an allowed artwork `content_type` and `contains_text: false`. The helper applies per-type dimensions, an 8% per-crop area ceiling, a 20% aggregate crop ceiling, a 25% embedded-raster ceiling, and rejects adjacent/overlapping crop tiles.
