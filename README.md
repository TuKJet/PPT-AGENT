...
With `on`, every version-3 `render-jobs/img-svg/slide-xx.json` contains the actual `source_image_path`, a fully compiled high-fidelity tracing prompt, prompt hash, conversion-evidence path, crop-strategy manifest, and fidelity-review paths. Pass the image and compiled prompt together in the same vision-model turn; the image is the sole visual truth and the task is tracing, not redesign. Inventory every visible element before conversion. Icons, illustrations, logos, photos, and textures default to tight source-image crops so their original appearance is preserved exactly; visible text, lines, boxes, dividers, arrows, and simple geometry remain SVG vectors. Crop entries declare `content_type` and `contains_text: false`; deterministic validation requires every source-specific artwork item to have a matching crop and rejects broad crops or crops that overlap vector text. Complete the evidence/manifests, then run `complete-img-svg` once to render source-comparison PNGs and pending review files. Inspect every source/render pair, repair drift, mark faithful pages passed, and rerun completion before export. Do not create `v1`, `v2`, overlay, image-elements, or clean SVG variants. The final `<topic>-img-svg.pptx` embeds native SVG media rather than rasterizing the reconstructed page.
