...
9. Every visible logo, icon, badge, illustration, and decorative symbol must remain. Faithfully trace it as paths or crop only the tight incompatible artwork region directly from the original IMG with Pillow and embed the crop as a Base64 PNG `<image>` node. Never crop a card/panel/title/chart band or any visible text, and never replace a source icon with a generic plus, checkmark, circle, user silhouette, arrow, or approximate library icon. Each crop must declare its artwork `content_type` and `contains_text: false`; keep it below 12% of the canvas and no wider than 520px or taller than 420px.
